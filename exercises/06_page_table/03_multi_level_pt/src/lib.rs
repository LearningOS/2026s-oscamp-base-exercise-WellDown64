...
impl Sv39PageTable {
        ...
        for level in (0..=2).rev() {
            ...
            let mut next_ppn = 0;
            {
                ...
                if level == 0 {
                    ...
                    node.entries[vpn] = (target_ppn << PPN_SHIFT) | flags;
                    break;
                }
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        let mut current_ppn = self.root_ppn;
        let vpn = [
            Self::extract_vpn(va, 0),
            Self::extract_vpn(va, 1),
            Self::extract_vpn(va, 2),
        ];
        for level in (0..=2).rev() {
            let pte = self.nodes.get(&current_ppn).unwrap().entries[vpn[level]];
            if pte & PTE_V == 0 {
                return TranslateResult::PageFault;
            }
            if level == 0 && (pte >> 1) & (0b111) == 0 {
                return TranslateResult::PageFault;
            }
            let new_ppn = pte >> PPN_SHIFT;
            if (pte >> 1) & (0b111) != 0 {
                if level == 0 {
                    let offset = va & 0xFFF;
                    let pa = new_ppn * PAGE_SIZE as u64 + offset;
                    return TranslateResult::Ok(pa);
                }
                if level == 1 {
                    let offset = va & 0x1FFFFF;
                    let pa = (new_ppn << 12) + offset;
                    return TranslateResult::Ok(pa);
                }
