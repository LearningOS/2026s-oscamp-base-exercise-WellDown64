...
impl<T> RwLock<T> {
    ...
    pub fn read(&self) -> RwLockReadGuard<'_, T> {
        // TODO
        while self.state.load(Ordering::Acquire) & (WRITER_HOLDING | WRITER_WAITING) != 0 {
            core::hint::spin_loop();
        }

        while self.state.load(Ordering::Acquire) & READER_MASK == READER_MASK {
            core::hint::spin_loop();
        }
    ...
    pub fn write(&self) -> RwLockWriteGuard<'_, T> {
        ...
        loop {
            while self.state.load(Ordering::Acquire) & (READER_MASK | WRITER_HOLDING) != 0 {
                core::hint::spin_loop();
            }
            ...
            // if a writer is just released, state == 0
