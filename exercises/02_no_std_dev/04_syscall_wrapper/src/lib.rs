...
pub fn sys_write(fd: usize, buf: &[u8]) -> isize {
    ...
    let ret;
    unsafe {
        ret = syscall3(NATIVE_SYS_WRITE, fd, buf.as_ptr().addr(), buf.len());
    }
    ret
}
...
pub fn sys_read(fd: usize, buf: &mut [u8]) -> isize {
    ...
    let ret;
    unsafe {
        ret = syscall3(NATIVE_SYS_READ, fd, buf.as_ptr().addr(), buf.len());
    }
    ret
}
...
pub fn sys_exit(code: i32) -> ! {
    ...
    unsafe {
        let code: usize = code.cast_unsigned().try_into().unwrap();
