...
pub fn run_command(program: &str, args: &[&str]) -> String {
    ...
    let output_stdout = String::from_utf8(command_output.stdout).unwrap();
    output_stdout
}
...
pub fn pipe_through_cat(input: &str) -> String {
    ...
    // child.stdin automatically dropped here because its ownership is taken by .take()
...
pub fn get_exit_code(command: &str) -> i32 {
    ...
    let code = Command::new("sh").args(["-c", command])
...
pub fn run_command_with_result(program: &str, args: &[&str]) -> io::Result<String> {
    ...
    let result =  String::from_utf8(output).map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e));
    result
}
...
pub fn pipe_through_grep(pattern: &str, input: &str) -> String {
    ...
    grep_process.stdin.take().unwrap().write_all(input.as_bytes()).unwrap();
    // TODO: Drop stdin to close pipe
    // stdin automatically dropped here
    // TODO: Read output from child stdout line by line
    let mut buf = String::new();
    grep_process.stdout.take().unwrap().read_to_string(&mut buf).unwrap();
